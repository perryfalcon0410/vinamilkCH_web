      ...
      for (let i = 0; i < 3; i += 1) {
        if (JSPM.JSPrintManager.websocket_status === JSPM.WSStatus.Open && i < 3) {
          ...
          if (jspmCheckStatus()) {
            ...
            pdf.addImage(logo, 'PNG', pageWidth * 0.17, 10)
            marginTop += spaceRowInCluster + 15
            ...
            pdf.autoTable({
              ...
              body: [
                [
                  { content: 'SP', styles: { halign: 'left', cellWidth: (pageWidth - 10) * 0.5 } },
                  { content: 'SL', styles: { halign: 'center', cellWidth: (pageWidth - 10) * 0.15 } },
                  { content: this.removeVietnameseTones('Giá', this.isRemoveVNTones), styles: { halign: 'center', cellWidth: (pageWidth - 10) * 0.15 } },
                  { content: this.removeVietnameseTones('T.Tiền', this.isRemoveVNTones), styles: { halign: 'right', cellWidth: (pageWidth - 10) * 0.3 } },
                ],
              ],
            })
            marginTop += 25
            ...
            this.printSalesReceiptData.products.forEach(product => {
              if (product.listOrderItems !== null) {
                product.listOrderItems.forEach(item => {
                  ...
                  pdf.text(`${this.$formatNumberToLocale(item.quantity)}`, ((pageWidth) * 0.5), marginTop, { align: 'right' })
                  pdf.text(`${this.$formatNumberToLocale(item.price)}`, ((pageWidth) * 0.70), marginTop, { align: 'right' })
              ...
              if (product.listFreeItems !== null) {
                product.listFreeItems.forEach(freeItem => {
                  ...
                  pdf.text(this.removeVietnameseTones(`${freeItem.productName}`, this.isRemoveVNTones), marginLeft + marginLeftColumnSL, marginTop, { align: 'right' })
                })
              }
            })
            ...
            pdf.text('- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -', marginLeft, marginTop)
            marginTop += 10
            const marginValueCalculate = pageWidth - 8
