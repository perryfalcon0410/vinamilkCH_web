          ...
          if (jspmCheckStatus()) {
            ...
            this.printSalesReceiptData.products.forEach(product => {
              if (product.listOrderItems !== null) {
                product.listOrderItems.forEach(item => {
                  ...
                  pdf.text(`${this.$formatNumberToLocale(item.totalPrice)}`, (columnWidth1 + columnWidth2 + columnWidth3 + columnWidth4) - 5, marginTop, { align: 'right' })
            ...
            if (this.printSalesReceiptData.lstZM !== null) {
              this.printSalesReceiptData.lstZM.forEach(zm => {
                ...
                pdf.text(`${this.$formatNumberToLocale(zm.amount)}`, (columnWidth1 + columnWidth2 + columnWidth3 + columnWidth4) - 5, marginTop, { align: 'right' })
              })
            }
            marginTop += spaceRowInCluster
            pdf.setFontSize(fontSize)
