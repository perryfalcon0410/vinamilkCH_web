      ...
      for (let i = 0; i < 3; i += 1) {
        if (JSPM.JSPrintManager.websocket_status === JSPM.WSStatus.Open && i < 3) {
          ...
          if (jspmCheckStatus()) {
            ...
            const pdf = new jsPDF('p', 'px', [heightPage, 287.24409449]) // portrait, heigth x width
            const marginLeft = 5
            let marginTop = 10
            ...
            if (this.printSalesReceiptData.orderNumber) {
              ...
              pdf.setFontType('bold')
              pdf.text(`${this.printSalesReceiptData.orderNumber}`, marginLeft + 30, marginTop)
              pdf.setFontType('normal')
              marginTop += spaceRowInCluster
            }
